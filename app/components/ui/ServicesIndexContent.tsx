...
import { servicesIndexCopy } from '@/lib/servicesIndexCopy';
  ...
  return (
    <div lang={lang}>
      ...
      <section className="py-16 sm:py-20">
        <div className="wrap">
          <FadeIn className="mb-10 flex flex-wrap gap-4">
            <Link
              href="/warranty"
              className="group flex min-w-[16rem] flex-1 items-center gap-4 rounded-2xl border border-primary-green/25 bg-primary-green/5 p-6 transition hover:border-primary-green/40 hover:bg-primary-green/8"
            >
              ...
              <span>
                <span className="block font-display text-2xl tracking-wide text-primary-blue">
                  {copy.featured.warranty.title}
                </span>
                <span className="mt-1 block text-sm text-ink-muted">{copy.featured.warranty.detail}</span>
              </span>
            </Link>
            <Link
              href="/services/engine-rebuilds-englewood-co"
              className="group flex min-w-[16rem] flex-1 items-center gap-4 rounded-2xl border border-primary-blue/20 bg-primary-blue/5 p-6 transition hover:border-primary-blue/35 hover:bg-primary-blue/8"
            >
              ...
              <span>
                <span className="block font-display text-2xl tracking-wide text-primary-blue">
                  {copy.featured.rebuilds.title}
                </span>
                <span className="mt-1 block text-sm text-ink-muted">{copy.featured.rebuilds.detail}</span>
              </span>
            </Link>
          </FadeIn>
        </div>
      ...
      {SERVICE_CATEGORIES.map((category, catIndex) => {
        const catCopy = copy.categories[category.label] ?? {
          ...
          >
            <div className="wrap">
              <FadeIn className="mb-10 max-w-3xl">
                <p className="text-xs font-semibold uppercase tracking-[0.28em] text-primary-green">{catCopy.label}</p>
                <h2 className="mt-3 font-display text-4xl tracking-wide text-foreground sm:text-5xl">{catCopy.label}</h2>
                <p className="mt-4 text-lg text-ink-muted">{catCopy.description}</p>
              </FadeIn>
              ...
              <Stagger className="grid gap-5 sm:grid-cols-2" stagger={0.06}>
                {category.slugs.map((slug) => {
                  ...
                  const isPremium =
                    slug === 'engine-rebuilds-englewood-co' || slug === 'camshaft-lifter-repair-englewood-co';
                  const name = localizedServiceName(slug, lang, service.name);
                  const description = localizedServiceDescription(slug, lang, service.description);
                  return (
                    <StaggerItem key={slug}>
                      ...
                      >
                        <div className="relative min-h-[180px] sm:min-h-[220px]">
                          <Image
                            src={service.image}
                            alt={`${name} at RKC Automotive in Englewood, CO`}
                            fill
                        ...
                        <div className="flex flex-col justify-center p-6 sm:p-8">
                          ...
                          <h3 className="mt-3 font-display text-3xl tracking-wide text-white sm:text-4xl">{name}</h3>
                          <p className="mt-2 text-sm text-white/70 sm:text-base">{description}</p>
                          <span className="mt-5 text-sm font-bold text-primary-green-light">{copy.learnMore}</span>
                        </div>
                      </Link>
                    </StaggerItem>
                  );
