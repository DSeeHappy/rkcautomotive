...
export default function BrandModelPicker({ brand, reliabilitySnapshot }: BrandModelPickerProps) {
  const scrutiny = reliabilitySnapshot.bullets.find((bullet) => bullet.label === 'Higher scrutiny');
  const colorado = reliabilitySnapshot.bullets.find((bullet) => bullet.label === 'Colorado angle');
    ...
    <StaggerItem className="rounded-2xl border border-white/20 bg-[#060a12]/45 p-5 backdrop-blur-sm">
      ...
      {scrutiny ? (
        <div className="mt-5">
          ...
          <p className="mt-2 text-sm leading-relaxed text-white/85">{scrutiny.text}</p>
        </div>
      ) : null}
      ...
      {colorado ? (
        <div className="mt-4">
          ...
          <p className="mt-2 text-sm leading-relaxed text-white/85">{colorado.text}</p>
        </div>
      ) : null}
