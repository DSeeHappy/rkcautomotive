/**
 * Targeted downloader for Jeep/Nissan/Hyundai BrandTabs images (avoids full-catalog Wikimedia rate limits).
 * Run: node scripts/sync-jeep-nissan-hyundai-images.mjs
 */
...
const TARGET_MAKES = new Set(['Jeep', 'Nissan', 'Hyundai']);
...
async function downloadAndConvert(sourceUrl, destPath, attempt = 1) {
  const response = await fetch(sourceUrl, {
    headers: { 'User-Agent': 'RKC-Automotive-Site/1.0 (jeep/nissan/hyundai image sync)' },
  });
...
console.log(`Jeep/Nissan/Hyundai download complete: ${downloaded} saved, ${skipped} skipped, ${failed} failed`);
