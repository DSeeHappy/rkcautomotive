 ...
 * Pass `lang: 'es'` for Spanish template chrome (client EN|ES toggle).
 */
...
function buildDeepDive(
  ...
  const t = vehicleCopy(lang).deepDive;
  const LOCAL = t.local;
  ...
  const realityParagraphs =
    lang === 'es'
      ...
      : [
          ...
          failureSnippet
            ? `Platform note for ${ctx.brandName} drivers: ${failureSnippet} RKC inspects for these patterns during every ${ctx.model} ${serviceLower} visit — not just the immediate symptom you came in for.`
  ...
  const symptoms: DeepDiveSymptomCard[] =
    lang === 'es'
      ? [
          {
            iconIndex: 0,
            title: `Señales de alerta específicas de ${serviceLower} del ${ctx.model}`,
            body: `Los conductores traen ${ctx.brandName} ${ctx.model} a RKC cuando aparecen síntomas de ${template.focus} bajo carga en Colorado — mañanas frías en Federal Blvd, pendientes en C-470 o calor en Santa Fe Drive. Una alerta temprana ahorra dinero. Correlacionamos su queja con datos en vivo e inspección física — no solo borrar un código genérico.`,
            ...accentAt(0),
          },
          {
            iconIndex: 1,
            title: 'Millaje, edad e intervalos de servicio severo',
            body: `Los intervalos del ${ctx.model} (${ctx.yearRange}) se acortan al remolcar, idle en tráfico o estacionar afuera en inviernos de Englewood. Colorado es servicio severo para refrigeración, frenos, baterías y fluidos. Si su ${ctx.model} pasó 60,000 millas sin ${serviceLower}, asuma que la inspección está vencida hasta verificar registros.`,
            warning: template.warning,
            ...accentAt(1),
          },
          {
            iconIndex: 2,
            title: 'Condición de fluidos, fugas y ciclos de calor',
            body: `Los cambios de temperatura en Colorado envejecen fluidos y sellos más rápido. En el ${ctx.brandName} ${ctx.model}, el fluido degradado aparece como estremecimiento, demora, olor o cambio de color mucho antes de una luz. RKC inspecciona nivel, color y olor, y recomienda cambio o reparación según medición — no solo millaje.`,
            ...accentAt(2),
          },
          {
            iconIndex: 3,
            title: 'Ruido, vibración y cambio de rendimiento',
            body: `Vibración nueva a velocidad de autopista, clic en giros cerrados o pérdida de potencia subiendo Lookout Mountain a menudo se debe a desgaste de ${template.focus}. Los ${vLabel}s ${ctx.brandName} ${ctx.model} ocultan problemas hasta que aumenta la carga. Probamos en carretera cuando es seguro y aislamos el componente antes de cotizar piezas.`,
            ...accentAt(3),
          },
        ]
      : [
          {
            iconIndex: 0,
            title: `Warning signs specific to ${ctx.model} ${serviceLower}`,
  ...
  const technicalCards: DeepDiveTechnicalCard[] =
    lang === 'es'
      ? [
          {
            title: `Contexto de plataforma ${ctx.brandName} ${ctx.model} (${ctx.yearRange})`,
            body: `Cada generación del ${ctx.model} tiene puntos débiles conocidos en ${template.focus}. RKC ajusta el alcance a su era de VIN — no un trabajo genérico. ${typeNote} Verificamos fecha de fabricación, equipo y tren motriz antes de recomendar piezas.`,
            ...accentAt(0),
          },
          {
          ...
          {
            title: 'Lo que cuesta aplazar el servicio',
            body: `Saltar ${serviceLower} en un ${ctx.brandName} ${ctx.model} convierte desgaste mantenible en fallas en cascada. El servicio preventivo en Englewood cuesta una fracción de grúa más reparación tras una falla en la carretera.`,
            ...accentAt(2),
          },
          {
            title: 'Cómo diagnostica RKC antes de cotizar',
            body: `Leemos datos del fabricante, medimos desgaste y fotografiamos hallazgos. Usted aprueba un presupuesto por escrito a ${LABOR_RATE} publicado antes de un desarmado mayor. Sin cargos sorpresa — llamamos primero. ${LOCAL}`,
            ...accentAt(3),
          },
        ]
      : [
  ...
  const processSteps: DeepDiveProcessStep[] =
    lang === 'es'
      ? [
          {
            step: '01',
            title: 'Recepción y documentación de síntomas',
            body: `Registramos la queja de su ${ctx.model}, millaje e historial. Para ${serviceLower}, anotamos cuándo aparecen los síntomas — arranque en frío, autopista, remolque o idle — porque las fallas ${ctx.brandName} suelen ser específicas de carga y temperatura.`,
          },
          {
            step: '02',
            title: 'Inspección y escaneo de datos',
            body: `Los técnicos inspeccionan componentes de ${template.focus} y escanean módulos por códigos pendientes. Datos en vivo, pruebas de presión y mediciones físicas vienen antes de recomendar piezas.`,
          },
          {
            step: '03',
            title: 'Presupuesto por escrito y aprobación',
            body: `Recibe un presupuesto línea por línea con piezas y mano de obra a ${LABOR_RATE}. Explicamos qué es urgente versus monitorear para que los dueños de ${ctx.model} controlen el presupuesto.`,
          },
          {
            step: '04',
            title: 'Ejecución de reparación o servicio',
            body: `RKC usa fluidos con especificación ${ctx.brandName} y piezas de calidad. Los procedimientos siguen torque y sangrado de fábrica — crítico para ${serviceLower} en ${vLabel}s modernos.`,
          },
          {
            step: '05',
            title: 'Verificación y prueba en carretera',
            body: `Verificamos el funcionamiento correcto, limpiamos adaptaciones solo cuando corresponde y probamos su ${ctx.model} en condiciones reales. La documentación queda en su factura.`,
          },
        ]
      : [
          {
            ...
            body: `We capture your ${ctx.model} complaint, mileage, and recent service history. For ${serviceLower}, we note when symptoms appear — cold start, highway, towing, or idle — because ${ctx.brandName} faults are often load and temperature specific.`,
          },
          {
            step: '02',
          ...
          },
          {
            ...
            body: `RKC uses ${ctx.brandName}-spec fluids and quality parts. Service procedures follow factory torque and bleed specifications — critical for ${serviceLower} on modern ${vLabel}s.`,
          },
          {
            step: '05',
  ...
  const checklistGroups: DeepDiveChecklistGroup[] =
    lang === 'es'
      ? [
          {
            category: `Alcance de ${template.serviceName}`,
            items: [
              `Inspeccionar y dar servicio a ${template.focus}`,
              `Documentar millaje y era de VIN del ${ctx.brandName} ${ctx.model}`,
              'Escanear módulos por códigos pendientes y permanentes',
              'Prueba en carretera cuando sea seguro para verificar resolución',
              'Restablecer indicadores de mantenimiento cuando aplique',
            ],
          },
          {
            category: 'Fluidos y filtros',
            items: [
              'Verificar que el tipo de fluido coincida con la especificación del fabricante',
              'Inspeccionar contaminación, olor y nivel',
              'Reemplazar filtros cuando el intervalo lo requiera',
              'Purgar aire de circuitos hidráulicos tras el servicio',
              'Eliminar fluidos residuales según regulaciones de Colorado',
            ],
          },
          {
            category: 'Hardware y desgaste',
            items: [
              'Medir componentes de desgaste antes de recomendar reemplazo',
              'Inspeccionar sellos, fuelles y bujes mientras hay acceso',
              'Apretar sujetadores al torque de fábrica',
              'Revisar soportes y protectores por corrosión',
              'Fotografiar hallazgos para el registro del cliente',
            ],
          },
          {
            category: 'Notas de Colorado',
            items: [
              'Recomendación de intervalo severo al remolcar o manejar en montaña',
              'Revisión de batería y carga cuando la carga eléctrica es alta',
              'Chequeo rápido de llantas y alineación si hay trabajo de suspensión',
              'Recordatorio de seguimiento pre-invierno en servicio de otoño',
              'Presupuesto por escrito retenido para garantía y terceros',
            ],
          },
        ]
      : [
  ...
  const faqItems: FAQItem[] =
    lang === 'es'
      ? [
          {
            question: `¿Cuánto cuesta ${serviceLower} del ${ctx.model} en Englewood?`,
            answer: `El costo depende del año del ${ctx.model}, el tren motriz y lo que encontremos al inspeccionar ${template.focus}. RKC da un presupuesto por escrito a ${LABOR_RATE} antes de trabajo mayor. Cotizamos su VIN específico, no un promedio nacional.`,
          },
          {
            question: `¿Con qué frecuencia debo agendar ${serviceLower} en un ${ctx.brandName} ${ctx.model}?`,
            answer: `Siga los intervalos de fábrica ${ctx.brandName} para modelos ${ctx.model} ${ctx.yearRange}, luego acórtelos para servicio severo en Colorado — remolque, montaña, estacionamiento exterior y tráfico en I-25. RKC imprime la próxima visita recomendada en su factura.`,
          },
          {
            question: `¿Cuáles son síntomas comunes del ${ctx.model} para ${serviceLower}?`,
            answer: `Señales comunes incluyen cambios de ruido, vibración, pedal, temperatura o luces relacionadas con ${template.focus}. ${template.warning} Agende diagnóstico en Englewood antes de que el síntoma lo deje varado.`,
          },
          {
            question: `¿Usan piezas OEM o del mercado de reposición en ${ctx.brandName} ${ctx.model}?`,
            answer: `RKC usa OEM o equivalentes de calidad que cumplen especificaciones ${ctx.brandName} para ${serviceLower}. Discutimos opciones en su presupuesto para equilibrar presupuesto y durabilidad.`,
          },
          {
            question: `¿Puedo manejar mi ${ctx.model} antes de completar este servicio?`,
            answer: `Depende de la gravedad. Algunos problemas permiten manejo local cuidadoso; otros — sobrecalentamiento, fade de frenos, luces intermitentes — significan deje de manejar y llame a RKC. Evaluamos su ${ctx.model} al recibir y le decimos con honestidad si es seguro para I-25.`,
          },
          {
            question: `¿Por qué elegir RKC para servicio del ${ctx.model} cerca de Denver?`,
            answer: `${LOCAL} Publicamos ${LABOR_RATE} de mano de obra, damos presupuestos por escrito y explicamos hallazgos específicos de ${ctx.brandName} en lenguaje claro. A menudo hay citas el mismo día si hay piezas en stock.`,
          },
          {
            question: `¿El clima de Colorado afecta ${serviceLower} del ${ctx.model}?`,
            answer: `Sí. Ciclos de congelación, altitud y sal aceleran el desgaste de ${template.focus}. ${typeNote} Las inspecciones pre-invierno y post-verano detectan problemas de fluidos y sellos.`,
          },
          {
            question: `¿Ofrecen garantía en reparaciones del ${ctx.model}?`,
            answer: `RKC respalda ${serviceLower} calificado con términos de garantía discutidos al aprobar el presupuesto. Documentamos piezas y mano de obra — llame al ${BUSINESS.phone} con cualquier seguimiento.`,
          },
        ]
      : [
          {
            question: `How much does ${ctx.model} ${serviceLower} cost in Englewood?`,
            answer: `Cost depends on ${ctx.model} year, drivetrain, and what we find during inspection of ${template.focus}. RKC provides a written estimate at our posted ${LABOR_RATE} labor rate before major work. Typical ${ctx.brandName} ${ctx.model} ${serviceLower} ranges vary with parts availability and severity — we quote your specific VIN, not a national average.`,
          },
          {
            question: `How often should I schedule ${serviceLower} on a ${ctx.brandName} ${ctx.model}?`,
          ...
          {
            question: `What are common ${ctx.model} symptoms for ${serviceLower}?`,
          ...
          },
          {
            ...
            answer: `RKC uses OEM or quality equivalent parts that meet ${ctx.brandName} specifications for ${serviceLower}. We discuss options on your estimate so you can balance budget and longevity — especially for ${vLabel}s that tow or commute daily across the Denver metro.`,
          },
          {
          ...
          {
            question: `Does Colorado weather affect ${ctx.model} ${serviceLower}?`,
          ...
          },
          {
            ...
            answer: `RKC stands behind qualified ${serviceLower} with warranty terms discussed at estimate approval. We document parts and labor so you have recourse if a related issue appears — call ${BUSINESS.phone} with any follow-up concern.`,
          },
        ];
      ...
      ? [
          {
            ...
            values: [
              template.focus.split(',')[0] ?? template.focus,
              'Medir desgaste y condición del fluido',
              'El servicio severo acorta intervalos',
            ],
            highlight: 1 as number | undefined,
          },
          {
            label: 'Hardware relacionado',
            values: ['Sellos, soportes, protectores', 'Inspeccionar con acceso', 'Sal y grit aceleran el desgaste'],
          },
          {
            label: 'Diagnóstico',
            values: ['Datos de escaneo del fabricante', 'Códigos pendientes y datos en vivo', 'La altitud afecta el trim de combustible'],
          },
          {
            label: 'Documentación',
            values: ['Presupuesto por escrito', 'Fotos cuando los hallazgos son visuales', 'Factura para garantía/reventa'],
          },
          {
            label: 'Seguimiento',
            values: ['Próximo intervalo impreso', 'Ítems a monitorear anotados', 'Revisión pre-invierno ofrecida'],
          },
        ]
      : [
          {
            ...
            values: [
              template.focus.split(',')[0] ?? template.focus,
              'Measure wear & fluid condition',
              'Severe service shortens intervals',
            ],
            highlight: 1 as number | undefined,
          },
          {
            label: 'Related hardware',
            values: ['Seals, mounts, shields', 'Inspect while accessible', 'Salt and grit accelerate wear'],
          },
          {
            label: 'Diagnostics',
            values: ['Manufacturer scan data', 'Pending codes & live data', 'Altitude affects fuel trim'],
          },
          {
            label: 'Documentation',
            values: ['Written estimate', 'Photos when findings are visual', 'Invoice for warranty/resale'],
          },
          {
            label: 'Follow-up',
            values: ['Next interval printed', 'Monitor items noted', 'Pre-winter re-check offered'],
          },
        ];
  ...
  return {
    ...
    path: ctx.path,
    heroEyebrow: t.heroEyebrow(ctx.brandName, ctx.model),
    heroTitle,
    heroDescription,
    primaryCtaLabel: t.primaryCta(ctx.model),
    realityQuote: template.realityQuote,
    realityParagraphs,
    symptomsEyebrow: t.symptomsEyebrow(ctx.model),
    symptomsTitle: t.symptomsTitle(ctx.model, serviceLower),
    symptomsIntro: t.symptomsIntro(ctx.brandName, ctx.model, template.focus),
    symptoms,
    technicalEyebrow: t.technicalEyebrow,
    technicalTitle: t.technicalTitle(ctx.brandName, ctx.model, serviceLower),
    technicalIntro: t.technicalIntro(ctx.yearRange, ctx.model),
    technicalCards,
    tableTitle: t.tableTitle(ctx.model),
    tableIntro: t.tableIntro(serviceLower, ctx.brandName, ctx.model),
    tableCaption: t.tableCaption(ctx.brandName, ctx.model, template.serviceName),
    tableColumns: [...t.tableColumns],
    tableRows,
    processEyebrow: t.processEyebrow,
    processTitle: t.processTitle(ctx.model, serviceLower),
    processIntro: t.processIntro(ctx.brandName, ctx.model),
    processSteps,
    checklistEyebrow: t.checklistEyebrow,
    checklistTitle: t.checklistTitle(ctx.model, serviceLower),
    checklistIntro: t.checklistIntro(ctx.brandName),
    checklistGroups,
    laborTitle: t.laborTitle(ctx.model, template.serviceName),
    laborDescription: t.laborDescription(
      ctx.brandName,
      ctx.model,
      serviceLower,
      LABOR_RATE,
    ...
    faqTitle: t.faqTitle(ctx.model, template.serviceName),
    faqIntro: t.faqIntro(ctx.brandName, ctx.model),
    faqItems,
    relatedServiceSlug: template.relatedServiceSlug,
    relatedTitle: t.relatedTitle(ctx.model),
    serviceAreaLabel: t.serviceAreaLabel(ctx.model, serviceLower),
    finalCtaTitle: t.finalCtaTitle(ctx.model, serviceLower),
